...
function buildDarwinMenu(): Menu {
  return Menu.buildFromTemplate([
    ...
    // Kept in production builds, not just dev: setting a custom application menu also drops
    // the default Cmd+Opt+I accelerator, which otherwise leaves a packaged build with no way
    // to open DevTools — and some bugs only reproduce there, since a downloaded build is a
    // different code-signing identity to macOS and so carries its own set of TCC grants.
    {
      label: "View",
